...
if (-not (Assert-Admin)) {
  ...
  try {
    ...
    Write-Host "Elevation prompt opened. Approve UAC to continue bridge setup."
    exit 0
  } catch {
...
& netsh interface portproxy add v4tov4 listenaddress=$listenAddress listenport=$BridgePort connectaddress=127.0.0.1 connectport=$ChromePort | Out-Null
...
Write-Host "WSL CDP bridge configured."
Write-Host "Listen address: $listenAddress"
Write-Host "Bridge: ${listenAddress}:$BridgePort -> 127.0.0.1:$ChromePort"
