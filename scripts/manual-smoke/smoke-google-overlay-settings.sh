...
SCENARIO="${1:-${SMOKE_SCENARIO:-meeting}}"
...
SMOKE_ALLOW_AUX_TABS="${SMOKE_ALLOW_AUX_TABS:-0}"
...
run_smoke() {
  ...
  echo "Running Google overlay settings smoke..."
  SMOKE_SINGLE_TAB="$SMOKE_SINGLE_TAB" \
    SMOKE_STEP_MODE="$SMOKE_STEP_MODE" \
    SMOKE_STEP_PAUSE_MS="$SMOKE_STEP_PAUSE_MS" \
    SMOKE_ALLOW_AUX_TABS="$SMOKE_ALLOW_AUX_TABS" \
    node "$REPO_ROOT/scripts/manual-smoke/smoke-google-overlay-settings.mjs" "$SCENARIO" "$PORT"
}
