...
RELOAD_PROVIDER_TABS="${RELOAD_PROVIDER_TABS:-1}"
...
SMOKE_LIVE_DIAGNOSTICS_MIN_LEVEL="${SMOKE_LIVE_DIAGNOSTICS_MIN_LEVEL:-trace}"
...
SMOKE_DIAGNOSTICS_MIN_LEVEL="${SMOKE_DIAGNOSTICS_MIN_LEVEL:-trace}"
...
echo "Running provider smoke preflight for localization DLS..."
SMOKE_SINGLE_TAB="$SMOKE_SINGLE_TAB" \
  SMOKE_STEP_MODE="$SMOKE_STEP_MODE" \
  SMOKE_STEP_PAUSE_MS="$SMOKE_STEP_PAUSE_MS" \
  SMOKE_DIAGNOSTICS_MIN_LEVEL="$SMOKE_DIAGNOSTICS_MIN_LEVEL" \
  node "$REPO_ROOT/scripts/manual-smoke/smoke-provider.mjs" "$PROVIDER" "$SCENARIO" "$PORT"

echo "Running localization DLS..."
SMOKE_STEP_MODE="$SMOKE_STEP_MODE" \
  SMOKE_STEP_PAUSE_MS="$SMOKE_STEP_PAUSE_MS" \
  SMOKE_DIAGNOSTICS_MIN_LEVEL="$SMOKE_DIAGNOSTICS_MIN_LEVEL" \
  node "$REPO_ROOT/scripts/manual-smoke/smoke-ui-i18n.mjs" "$PROVIDER" "$SCENARIO" "$PORT"
