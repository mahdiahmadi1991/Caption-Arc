...
export const koMessages = {
    ...
    options: {
        ...
        label: "보관 보존 기간",
        description:
          "CaptionArc가 종료되었고 별표가 없는 세션을 로컬 보관함에서 제거하기 전에 얼마나 오래 유지할지 선택합니다. 끔을 선택하면 모든 자동 보관 삭제가 비활성화됩니다.",
        off: "끔",
        days: "{count}일",
        oneYear: "1년",
      },
      storeMeetingChat: {
        ...
        models: {
            ...
            gpt5: {
                description: "Mini나 Nano보다 더 강한 범용 품질이 필요하지만 GPT-5.2까지는 원하지 않을 때 쓰는 주력 GPT-5 모델입니다.",
                badge: "주력"
            },
            gpt5Nano: {
            ...
            gpt41Nano: {
                description: "비용과 지연 시간이 가장 중요한 초경량 작업을 위한 가장 작은 GPT-4.1 옵션입니다."
            },
        },
        help: {
          apiKey: `이 장치에서 CaptionArc가 사용할 OpenAI API 키를 입력합니다.

- 이 키는 이 장치에만 남고 동기화나 백업 내보내기에는 포함되지 않습니다.
- 자신의 접근 정책과 과금 방식에 맞는 키를 사용하는 것이 좋습니다.
- 검증이 실패하면 키 상태, 프로젝트 크레딧, 모델 접근 권한을 확인하세요.`,
          model: `라이브 번역, 요약, 어시스턴트 출력에 사용할 OpenAI 모델을 고릅니다.

- 가벼운 모델은 보통 회의 중 응답이 더 빠릅니다.
- 강한 모델은 보통 더 좋은 문장을 만들지만 더 느리고 비용이 높을 수 있습니다.
- 확실하지 않다면 추천 기본값을 유지하세요.

예: 라이브 회의에는 빠른 모델을 쓰고, 요약 품질이 더 중요할 때만 더 강한 모델로 바꾸세요.`,
          uiLanguage: `이 설정은 CaptionArc 인터페이스 자체의 언어를 바꿉니다.

- 설정 화면, 회의 기록, 빠른 화면, 회의 중 UI 문구에 영향을 줍니다.
- 이것만으로 실시간 번역이나 요약의 출력 언어가 바뀌지는 않습니다.
- CaptionArc가 환경을 자동으로 따르길 원하면 브라우저/시스템 옵션을 사용하세요.`,
          translationInstructions: `이 지침은 모든 라이브 번역 요청과 함께 전송됩니다.

- 짧고 구체적으로 작성하세요.
- 용어, 톤, 자막 정리 규칙에 사용하세요.
- 너무 긴 프롬프트는 라이브 번역을 느리고 불안정하게 만들 수 있습니다.

예: \`제품 이름은 영어로 유지하고, 짧은 자막 스타일 문장을 우선합니다.\``,
          captureStartupBehavior: `CaptionArc가 지원되는 회의를 감지했을 때 무엇을 할지 정합니다.

- **묻기** 는 캡처 시작 전에 확인을 보여줍니다.
- **항상** 은 추가 확인 없이 바로 캡처를 시작합니다.
- **끔** 은 직접 켤 때까지 캡처를 대기 상태로 둡니다.

더 안전한 기본값이 필요하면 **묻기** 가 적합합니다.`,
          captionActivationBehavior: `회의 자막을 어떻게 켤지 정합니다.

- **가이드형** 은 마지막 단계를 직접 하도록 남겨 둡니다.
- **자동** 은 제공자 UI가 허용하면 자막을 자동으로 켜려고 시도합니다.
- 자동 모드는 회의 화면을 대신 조작하므로 더 민감한 설정입니다.

명확한 필요가 없다면 **가이드형** 을 권장합니다.`,
          sessionContinuationWindow: `같은 회의에 다시 들어왔을 때 CaptionArc가 동일한 세션을 얼마나 오래 이어갈지 정합니다.

- 짧은 시간은 새 세션을 더 자주 만듭니다.
- 긴 시간은 관련된 재입장을 같은 세션에 묶어 둡니다.
- \`0분\` 은 다시 들어올 때마다 새 세션을 시작한다는 뜻입니다.

예: \`120분\` 으로 두면 10:00에 끝난 회의도 같은 회의에 12:00까지 다시 들어오면 같은 세션으로 이어질 수 있습니다.`,
          overlayClickThrough: `이 설정은 회의 중 오버레이가 마우스 입력을 가로챌지, 아래 회의 화면으로 통과시킬지를 정합니다.

- 클릭이 오버레이를 지나 실제 회의 화면으로 가야 할 때 켜세요.
- 오버레이 위의 CaptionArc 컨트롤을 직접 조작해야 할 때 끄세요.
- 클릭 스루는 수동으로 읽기만 할 때는 편하지만, 컨트롤을 자주 바꾸면 덜 편할 수 있습니다.`,
          meetingArchiveRetention: `로컬 아카이브에서 종료되었고 별표가 없는 세션을 자동으로 정리하는 기간을 정합니다.

- 짧은 기간은 오래된 기록을 더 빨리 지웁니다.
- 긴 기간은 장치에 더 많은 기록을 남깁니다.
- **끔** 은 모든 자동 아카이브 삭제를 비활성화합니다.

별표가 있는 세션은 계속 자동 정리에서 보호됩니다.`,
          storeMeetingChat: `지원되는 회의 채팅을 저장된 회의 기록의 일부로 포함할지 정합니다.

- 켜면 채팅이 기록, 내보내기, 요약, 어시스턴트 컨텍스트에 포함될 수 있습니다.
- 채팅에는 보이는 자막보다 더 민감한 이름, 결정, 링크가 들어 있는 경우가 많습니다.
- 채팅 저장이 정책과 기밀 요구에 분명히 맞을 때만 켜세요.`,
          meetingOutputLanguage: `요약과 Meeting AI 응답 같은 회의 출력의 기본 언어를 정합니다.

- 이 설정은 인터페이스 언어와 라이브 번역과는 별개입니다.
- 보통 최종 결과를 읽고 싶은 언어를 선택하세요.
- 필요하면 나중에 출력 언어를 바꿀 수도 있습니다。`,
          profileName: `이 이름은 설정 화면과 이후 AI 워크플로에서 회의 프로필을 식별합니다.

- 프로필 목록에서 빠르게 알아볼 수 있도록 짧게 유지하세요.
- 일회성 회의 이름이 아니라 회의 유형의 이름을 적으세요.
- 좋은 이름은 이 프로필을 언제 다시 써야 하는지 바로 알 수 있게 해줍니다.

예: 고객 발견, 주간 팀 싱크, 이사회 업데이트.`,
          profileDescription: `이 설명은 이 회의 프로필을 언제 써야 하는지 빠르게 떠올릴 수 있게 도와주는 사람 중심의 맥락입니다.

- 회의 목적, 대상, 반복 리듬을 짧고 구체적으로 적으세요.
- 한눈에 올바른 프로필을 알아볼 수 있도록 짧고 명확하게 유지하세요.
- 여기는 프로필 메타데이터 자리이지, 긴 AI 지침을 쓰는 곳이 아닙니다.

예: 제품, 디자인, 엔지니어링 리드가 함께하는 주간 크로스펑셔널 리뷰.`,
          autoSummary: `회의가 끝났을 때 이 프로필이 자동으로 요약을 만들지 결정합니다.

- 거의 항상 요약이 필요한 반복 회의라면 켜 두세요.
- 필요할 때만 요약하려면 꺼 두세요.
- 자동 요약도 유효한 OpenAI 설정과 저장된 회의 데이터가 필요합니다.`,
          summaryEffort: `이 프로필의 요약에 AI가 얼마나 많은 작업을 쓰는지 정합니다.

- 적은 노력은 더 빠르고 더 저렴합니다.
- 많은 노력은 긴 회의나 정리가 덜 된 회의에 더 잘 맞습니다.
- 확실하지 않다면 **균형형** 이 가장 무난한 기본값입니다.`,
          summaryInstructions: `이 지침은 이 프로필의 요약 스타일을 정합니다.

- 구조, 대상 독자, 반복 용어를 지정할 때 사용하세요.
- 라이브 어시스턴트 동작이 아니라 요약 출력에만 집중하세요.
- 긴 정책 문서보다 짧고 구체적인 지침이 보통 더 잘 작동합니다.

예: \`먼저 경영진 요약을 쓰고, 그다음 담당자와 함께 액션 아이템을 정리해 주세요.\``,
          assistantEnabled: `이 프로필에서 Meeting AI를 켜거나 끕니다.

- 꺼져 있어도 나머지 Meeting AI 설정은 보이지만 라이브 가이드에는 영향을 주지 않습니다.
- 라이브 제안이 실제로 도움이 되는 회의 유형에서만 켜세요.
- 프로필마다 다른 Meeting AI 기본값을 가질 수 있습니다.`,
          assistantResponseIntent: `이 프로필에서 Meeting AI가 우선할 핵심 역할을 정합니다.

- 답변, 코칭, 요약, 위험 포착 중 어디에 더 무게를 둘지 정합니다.
- 말투만 바뀌는 것이 아니라 제안의 방향 자체가 달라집니다.
- 그 회의 유형에서 자주 필요한 도움 방식에 맞는 intent를 고르세요.`,
          assistantResponseFormat: `Meeting AI 응답의 형태를 정합니다.

- 짧은 bullet 형식은 회의 중 빠르게 훑어보기에 좋습니다.
- 더 말하기 쉬운 형식은 그대로 말로 옮기고 싶을 때 좋습니다.
- 시간 압박 속에서 가장 쓰기 쉬운 형식을 선택하세요.`,
          assistantResponseDepth: `Meeting AI 응답을 얼마나 짧게 또는 얼마나 풍부하게 만들지 정합니다.

- 낮은 깊이는 속도와 빠른 스캔에 유리합니다.
- 높은 깊이는 더 많은 맥락과 이유를 주지만 라이브 상황에서는 무겁게 느껴질 수 있습니다.
- 빠른 통화에는 가볍게, 전략 회의에는 더 깊게 두는 편이 좋습니다.`,
          assistantResponseTone: `Meeting AI 문구의 기본 톤을 조정합니다.

- 톤에 따라 제안이 더 직접적이거나 중립적이거나 외교적으로 느껴질 수 있습니다.
- 핵심 사실은 바꾸지 않지만 전달되는 느낌은 바뀝니다.
- 개인 취향보다 회의의 사회적 맥락에 맞추는 편이 좋습니다.`,
          assistantDeliveryBias: `빠른 출력과 더 완전한 가이드 사이의 균형을 정합니다.

- 타이밍이 가장 중요하면 빠른 전달이 더 낫습니다.
- 지연보다 뉘앙스가 중요하면 더 풍부한 출력이 낫습니다.
- 잘 모르겠다면 중간 설정을 유지하세요.`,
          assistantTriggerPolicy: `회의 중 어느 시점에 Meeting AI가 제안을 내야 하는지 정합니다.

- 보수적인 정책은 노이즈를 줄입니다.
- 더 적극적인 정책은 제안을 늘리지만 방해도 늘 수 있습니다.
- 필요한 순간에 충분히 도움이 되는 가장 낮은 수준을 선택하세요.`,
          assistantParticipantScope: `누구의 발화나 활동이 Meeting AI 제안에 영향을 줄지 정합니다.

- 좁은 범위는 내 역할에 중요한 사람에게 집중하게 해 줍니다.
- 넓은 범위는 방 전체의 흐름을 읽고 싶을 때 유리합니다.
- 어시스턴트가 너무 시끄럽거나 산만하다면 범위를 좁히세요.`,
          assistantInstructions: `이 지침은 이 프로필에서 Meeting AI 동작을 맞춤화합니다.

- 응답 스타일, 반복 제약, 도메인 기대치를 적는 데 사용하세요.
- 요약 지침과 라이브 번역 지침과는 분리하세요.
- 예외를 길게 늘어놓기보다 몇 가지 지속적인 규칙이 보통 더 잘 작동합니다.

예: \`간결한 말하기 포인트를 우선하고, 답변을 쓰기 전에 숨은 리스크를 먼저 드러내세요.\``,
        },
        translation: {
            bestFor: {
                title: "다음에 가장 적합",
        ...
        dataRecovery: {
            backupFile: {
                title: "암호화된 백업 파일",
                description: "내보낸 백업에는 공유 설정, 회의 프로필, 저장된 회의 세션, 전사, 채팅 기록, 번역 및 요약이 포함됩니다. OpenAI API 키 같은 기기 로컬 비밀 정보는 백업에 포함되지 않습니다. 클라우드 동기화를 사용할 수 없거나 휴대 가능한 암호화 스냅샷이 필요할 때 사용하세요.",
                export: "모든 데이터 내보내기",
                import: "백업 파일 가져오기"
            },
            passphrase: {
            ...
            deleteArchive: {
                title: "저장된 아카이브 삭제",
                syncedDescription: "이 기기, 연결된 클라우드 제공자 및 다른 동기화된 기기에서 동기화된 아카이브를 삭제합니다. OpenAI 설정, 환경설정 및 회의 프로필은 그대로 유지됩니다.",
                localDescription: "로컬 저장소에서 저장된 회의 세션을 모두 제거합니다. 그러면 OpenAI 설정, 환경설정 및 회의 프로필은 유지됩니다."
            },
            confirmDelete: {
        ...
        runtime: {
            ...
            cloudSync: {
                ...
                connectingProvider: "{provider} 로그인 시작 중...",
                connectHint: "안전한 브라우저 창이 열려야 합니다. 그곳에서 로그인을 완료한 뒤 여기로 돌아오세요.",
                connectSuccess: "{provider}가 연결되었습니다.",
                connectSuccessHint: "다음 동기화 주기부터 백그라운드 보호를 계속할 수 있습니다.",
                disconnecting: "클라우드 제공업체 연결 해제 중...",
                disconnectingProvider: "{provider} 연결을 해제하는 중...",
                disconnectHint: "클라우드 접근을 제거하는 동안 로컬 아카이브는 이 장치에 그대로 유지됩니다.",
                disconnectSuccess: "{provider} 연결이 해제되었습니다.",
                disconnectSuccessHint: "로컬 아카이브는 이 장치에서 계속 사용할 수 있습니다.",
                retrying: "클라우드 동기화를 다시 시도하는 중...",
                retryingProvider: "{provider}에 대한 다른 동기화 시도를 대기열에 추가하는 중...",
                retryHint: "CaptionArc가 공급자에게 백그라운드 동기화를 다시 재개하도록 요청합니다.",
                retrySuccess: "새 동기화 시도가 대기열에 추가되었습니다.",
                retrySuccessHint: "다음 실행에서 해당 공급자를 백그라운드로 다시 시도합니다.",
                reconnecting: "클라우드 제공업체 액세스 새로 고침 중...",
                reconnectingProvider: "{provider} 접근을 새로 고치는 중...",
                reconnectSuccess: "{provider} 접근이 새로 고쳐졌습니다.",
                resolvingChoice: "공유 설정 선택을 적용하는 중...",
                resolveChoiceHint: "다음 백그라운드 주기는 선택한 시작 지점에서 조정을 진행합니다.",
                choiceSuccess: "공유 설정 선택이 적용되었습니다.",
                choiceSuccessHint: "클라우드 동기화는 선택한 공유 설정 기준에서 계속됩니다.",
                refreshing: "최신 클라우드 동기화 상태를 확인하는 중...",
                refreshHint: "대기열 상태, 공급자 체크포인트, 최근 동기화 활동을 읽는 중입니다.",
                refreshSuccess: "클라우드 동기화 상태가 새로 고쳐졌습니다.",
                refreshSuccessHint: "최신 공급자 상태와 대기열 상태가 이제 이 페이지에 표시됩니다."
            }
        }
    },
    history: {
